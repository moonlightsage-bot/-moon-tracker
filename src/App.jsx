...
function App() {
  ...
  const getElementalAlchemy = (phaseQuality, signElement, signName, phaseName) => {
    // Now we focus on Sign Element + Phase Quality (not phase "element" which doesn't exist)
    
    const alchemyBySign = {
      "Fire": {
        alchemy: "Flames in flux — passion meets the lunar rhythm",
        qualities: {
          "Waning Crescent": "Fire dissolving into embers — intense insights composted into wisdom before renewal"
        }
      },
      "Water": {
        alchemy: "Deep waters and tides — emotion flows with Luna's pull",
        qualities: {
          "Waning Crescent": "Fertile depths — where shadow becomes soil, and endings dissolve into beginnings"
        }
      },
      "Earth": {
        alchemy: "Ground and cycles — matter responds to celestial time",
        qualities: {
          "Waning Crescent": "Composting matter — the body remembers, releasing what's complete to feed the next season"
        }
      },
      "Air": {
        alchemy: "Breath and phases — thought shifts with lunar light",
        qualities: {
          "Waning Crescent": "Final whispers — insights integrate in silence before the next breath of creation"
        }
      }
    }
    ...
    const getMeaning = () => {
      ...
      // Generic but accurate structure for other combinations
      return [
        `${signName}'s ${signElement.toLowerCase()} nature colors the moonlight`,
        `${phaseName}'s stage: ${phaseQuality.toLowerCase()}`,
        "Two rhythms harmonizing in this moment",
        "Sign flavor meets cycle timing"
      ]
    }
  ...
  const getMoonImagePath = (phaseName) => {
    const imageMap = {
      "New Moon": "/moon-phases-real/new-moon.png",
      "Waxing Crescent": "/moon-phases-real/waxing-crescent.png",
      "First Quarter": "/moon-phases-real/first-quarter.png",
      "Waxing Gibbous": "/moon-phases-real/waxing-gibbous.png",
      "Full Moon": "/moon-phases-real/full-moon.png",
      "Waning Gibbous": "/moon-phases-real/waning-gibbous.png",
      "Last Quarter": "/moon-phases-real/last-quarter.png",
      "Waning Crescent": "/moon-phases-real/waning-crescent.png"
    }
    return imageMap[phaseName] || "/moon-phases-real/full-moon.png"
  }
      ...
      <main>
        ...
        <div className="elemental-alchemy">
          ...
          <div className="alchemy-layers">
            ...
            <div className="alchemy-layer">
              <strong>Sign ({moonData.zodiacSign.element}):</strong> {moonData.zodiacSign.name} colors the moonlight with {moonData.zodiacSign.element.toLowerCase()}'s qualities
            </div>
            <div className="alchemy-layer">
              <strong>Modality ({moonData.zodiacSign.modality}):</strong> {moonData.zodiacSign.modality} {moonData.zodiacSign.element.toLowerCase()} shapes how this energy moves
            </div>
          </div>
        </div>
